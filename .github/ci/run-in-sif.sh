...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-tex's —
...
# so only scitex-tex's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_tex-$V"
rm -rf "$TMPDIR"
...
# scitex-tex's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-tex + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-tex's historical bare-uv/pip workflow so a
...
# scitex-tex's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-tex style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-tex-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-tex's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_tex --cov-report=xml --cov-report=term \
    -p no:cacheprovider
